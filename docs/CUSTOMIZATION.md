...
## 6. Smoke-test перед сдачей клиенту

Перед тем как отдать бота — пройди вручную в Telegram:

**Как клиент** (с обычного аккаунта):
- [ ] `/start` → главное меню с брендом нового профиля.
- [ ] «Оставить заявку» → выбор категории → ответ на все вопросы → загрузка
      файла (если есть) → ввод контакта → confirm → готово.
- [ ] На confirm-экране нажми «✏ Изменить ответы» → выбери один ответ →
      измени его → «✅ Готово» → confirm должен показать новый ответ.
- [ ] «Мои заявки» — заявка должна быть в списке со статусом «🆕 Новая».
- [ ] «FAQ» — все вопросы должны открываться.
- [ ] «Связаться с менеджером» — после нажатия одно сообщение с
      запросом контакта (не два).
- [ ] `/menu` посреди любого экрана — должен вернуть в главное меню.

**Как админ** (с ID из `ADMIN_IDS`):
- [ ] `/admin` → админ-меню.
- [ ] «📋 Заявки» → видишь заявку клиента.
- [ ] Открой её → нажми «📎 Показать файлы» (если приложены) → бот
      пришлёт альбом.
- [ ] Поменяй статус на «📞 Связались» → клиент получит уведомление в
      ЛС бота.
- [ ] Добавь «💬 Ответ клиенту» → клиент получит сообщение.
- [ ] «📊 Статистика» → видна цифра, кнопка «⬅ Назад» возвращает в
      админ-меню.
- [ ] «📤 CSV» → файл выгружается.

**Если webhook включён** (`WEBHOOK_URLS` задан):
- [ ] На каждое из действий выше приёмник получает POST с
      `X-Webhook-Signature` и валидной HMAC-подписью.

**Если SMTP включён** (`SMTP_HOST` задан):
- [ ] При создании заявки на адреса из `SMTP_ADMIN_EMAILS` приходит
      письмо.

Любой провал — открой [`DEPLOY.md`](DEPLOY.md) раздел Troubleshooting и
`make prod-logs`.
