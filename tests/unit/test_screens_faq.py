...
def test_render_faq_lists_questions():
    ...
    faq_buttons = [
        btn for row in screen.keyboard.inline_keyboard for btn in row if "faq:" in btn.callback_data
    ]
...
def test_render_faq_answer_invalid_index_raises():
    with pytest.raises(IndexError):
        render_faq_answer(content=_content(), index=999, stack=["main_menu", "faq", "faq_answer"])
