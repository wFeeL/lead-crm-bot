...
def test_lead_category_empty():
    screen = render_lead_category(content=_content(), categories=[], stack=["main_menu", "lead_category"])
...
def test_lead_category_with_items_has_button_per_category():
    ...
    screen = render_lead_category(content=_content(), categories=cats, stack=["main_menu", "lead_category"])
    btns = [b for row in screen.keyboard.inline_keyboard for b in row if "lead_cat:" in b.callback_data]
    assert len(btns) == 2
