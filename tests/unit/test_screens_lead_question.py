...
def _q(qid=1, text="Q?", qtype=QuestionType.TEXT, required=True, options=None):
    return {"id": qid, "key": "k", "text": text, "type": qtype, "required": required, "options": options or []}


def test_required_question_has_no_skip():
    q = _q(required=True)
    screen = render_lead_question(content=_content(), question=q, index=0, total=2, stack=["main_menu", "lead_category", "lead_question"])
...
def test_back_button_visible_when_index_gt_0():
    q = _q()
    screen = render_lead_question(content=_content(), question=q, index=1, total=3, stack=["main_menu", "lead_category", "lead_question"])
    labels = [b.text for row in screen.keyboard.inline_keyboard for b in row]
...
def test_choice_question_has_option_buttons():
    ...
    screen = render_lead_question(content=_content(), question=q, index=0, total=1, stack=["main_menu", "lead_category", "lead_question"])
    choice_btns = [b for row in screen.keyboard.inline_keyboard for b in row if "lq_choice:" in b.callback_data]
    assert len(choice_btns) == 3


def test_screen_id_and_progress_in_text():
    q = _q()
    screen = render_lead_question(content=_content(), question=q, index=2, total=5, stack=["main_menu", "lead_category", "lead_question"])
